...
-   Try to make functions, consider

-   Look at the leave out peptide targets

-   Look intro why taking the mean of the mean

-   Look at selected_peptrides\$peptide_data

```{r}
# Select top 20 most upregulated peptide targets
most_upregulated <- expression_meta_data_analysis_1 |> 
  filter(!is.na(mean_expression)) |> # Remove rows with NA
  group_by(mean_expression) |>        # Group by peptide_target
  slice_max(order_by = mean_expression, n = 1) |> # Take the row with max mean_expression for each peptide_target
  ungroup() |>                       # Ungroup to avoid grouped output
  slice_max(order_by = mean_expression, n = 20)  # Select the 20 highest mean_expression

# Select bottom 20 most downregulated peptide targets
most_downregulated <- expression_meta_data_analysis_1 |> 
  filter(!is.na(mean_expression)) |> # Remove rows with NA
  group_by(mean_expression) |>        # Group by peptide_target
  slice_min(order_by = mean_expression, n = 1) |> # Take the row with min mean_expression for each peptide_target
  ungroup() |>                       # Ungroup to avoid grouped output
  slice_min(order_by = mean_expression, n = 20)  # Select the 20 lowest mean_expression

# Combine the two subsets
selected_peptides <- bind_rows(most_upregulated, most_downregulated)
...
-   Our expression is normalized and here we look at the peptide levels'
