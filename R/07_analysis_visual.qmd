...
```{r}
meta_data |> 
  filter(is.na(years_smoked.exposures)) |> 
  nrow()
...
As the amount of significant differentially expressed proteins between alive and dead patients were found to be a relatively low number and thus we can visualize their individual expression levels.

The 6 most differentially expressed proteins between the two different vital status sorted on different ages
