...
Loading the data sets that we need for the Principal Component Analysis.
...
Transposing the dataframe so the data is in the correct format for doing a PCA on the protein expression. This is done using pivot_longer() and pivot_wider(). NA's are dropped.
...
Selecting numeric values in the data for computing the PCA analysis. NA's are dropped. The data is centered and scaled before doing the PCA.
...
Making a scree plot using using the tidy() function to extract the output from the prcom() function that performed the PCA analysis.
...
Joining the data frame used in the PCA with the meta_data. We do this to be able to color the datapoints in the plot based on vital status and pathological stage. We are using left join, as we only want the meta data for the patients that we have in our PCA.
...
Adding the metadata to the PCA:
...
Creating labels for the PCA plot, containing the variance percentage. The value is computed using tidy() and mutate(). The variance percentage is riunded to 2 decimal points. The labels are then pulled.
...
Making the PCA plot using PC1 and PC2 for the x and y axis respectively.
...
For both the PCA plot where data points are colored according to vital status and for the PCA plot where data points are colored according to vital status, we see that there is no grouping of the data points or tendencies that indicate that one group of the data varies from another group.

If we had succeded in performing the PCA analysis on the grouped data, eg. for pathological stage, we might have seen some gruping of data points in the PCA, that would show if there was a variance in the protein expression between the different pathological stages.
