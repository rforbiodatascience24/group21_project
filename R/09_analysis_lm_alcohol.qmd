...
Want to make a linear model of the peptide expression levels and if the patient has a alcohol history or not
...
We use the tidy() function to get the output from the linear model into a tidy dataframe. We specify that we want to include the 95% confidence intervals.
...
Term describes if the patient drinks alcohol. 0 = no and 1 = yes. Therefore the intercept the patient does not drink alcohol and the slope + intercept the patient does drink alcohol. And the slope is the diffrence between the two, therefore we select for the term = "alcohol_history_binomial" as that is the slope of the linear model.
...
From the linear model it has been found that 1 protein "ANNEXINVII" is over-expressed in the patients that have a history with drinking alcohol.
