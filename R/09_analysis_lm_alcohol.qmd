...
>>>>>>> 2464e0ace8c0659672914fcfa9f1afa7a5cb9155
#| echo: TRUE
#| eval: TRUE 
#| output: FALSE  
...
expression_meta_data_analysis6_alcohol <- expression_meta_data_aug |> 
  select(alcohol_history_binomial, 
         peptide_target,
         expression) |> 
   drop_na(expression)

expression_meta_data_analysis6_alcohol 
```
...
expression_meta_data_analysis6_alcohol_nested <- expression_meta_data_analysis6_alcohol |> 
  group_by(peptide_target) |> 
  nest() 

expression_meta_data_analysis6_alcohol_nested
```
...
expression_meta_data_analysis6_alcohol_nested_model <- expression_meta_data_analysis6_alcohol_nested |> 
  mutate(model_object = map(.x = data,
                            .f = ~lm(formula = expression ~ alcohol_history_binomial,
                                   data = .x)))

expression_meta_data_analysis6_alcohol_nested_model
```
...
expression_meta_data_analysis6_alcohol_nested_model_tidy <- expression_meta_data_analysis6_alcohol_nested_model |> 
  mutate(model_object_tidy = map(.x = model_object,
                                 .f = ~ tidy(conf.int = TRUE,
                                             conf.level = 0.95,
                                             x =.x)))

expression_meta_data_analysis6_alcohol_nested_model_tidy
```
...
expression_meta_data_analysis6_alcohol_estimates <- expression_meta_data_analysis6_alcohol_nested_model_tidy |>
  unnest(model_object_tidy)

expression_meta_data_analysis6_alcohol_estimates
...
expression_meta_data_analysis6_alcohol_estimates_clean <- expression_meta_data_analysis6_alcohol_estimates |> 
...
expression_meta_data_analysis6_alcohol_estimates_clean
```
...
expression_meta_data_analysis6_alcohol_estimates_clean <- expression_meta_data_analysis6_alcohol_estimates_clean |> 
...
expression_meta_data_analysis6_alcohol_estimates_clean |>  filter(is_significant == "yes")


```

<<<<<<< HEAD
We have 1protein where the expression levels are found to be significantly different between patients that drink and patients that does not.
=======
We have 1 protein where the expression levels are found to be significantly different between patients that drink and patients that does not.
>>>>>>> 2464e0ace8c0659672914fcfa9f1afa7a5cb9155
...
volcano_plot_alcohol <- expression_meta_data_analysis6_alcohol_estimates_clean |>
